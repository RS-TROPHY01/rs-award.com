  ...
  <footer class="shadow bg-gray-900 bottom-0">
    ...
    >
      <div class="text-center mx-auto my-auto col-span-1">
        <h5 class="text-center p-2 font-bold text-2xl text-white">
          สินค้าอื่นๆ
        </h5>
        <div v-for="product in otherProducts" :key="product.name" class="p-1">
          <a :to="product.href" class="hover:text-blue-500">
            <h6 class="font-bold text-xl text-white">{{ product.name }}</h6>
          </a>
        </div>
      </div>
      <div class="text-center mx-auto my-auto col-span-1">
        <h5 class="text-center p-2 font-bold text-2xl text-white">เบอร์โทร</h5>
